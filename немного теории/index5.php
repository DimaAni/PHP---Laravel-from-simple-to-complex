...
<hr>
<br>
<h2>Вспомним что проходили в этой главе коротко</h2>
<ul>
    <li>
        Ф-я представляет собой именованую часть кода и его можно вызвать по имени.  
        Пишем function затем в () указываем аргументы если это требуется. Далее в {}
        записываем необходимые команды.
    </li>
    <li>
        Ф-я может возвращать результат.Просто после return указываем необходимое. 
    </li>
    <li>
        Ф-ии в явном виде можно указать тип аргумента и тип результата.
    </li>
    <li>
        По умолчанию ф-ии передаются аргументы по значению. Создаются копии пере-х
        указанных аргументами ф-ии. Для передачи аргументов по ссылке и описании ф-ии
        перед именем соответствующего аргумента ставим знак $ .
    </li>
    <li>
        Аргументы могут иметь значения по умолчанию. Если при вызове ф-и аргументы не указаны
        то значения ипользуются по умолчанию.
    </li>
    <li>
        Ф-ии можно создавать в проивольным количеством аргументов. Для этого ставвим троеточие ...
        В описании функции.Обрабатывается такой аргумерт как массив.
    </li>
    <li>
        Рекурсия - это функция вызывающяя сама себя.
    </li>
    <li>
        Ф-я eval() - позволяет выполнять команды содержащиеся в тексте.(Говорят опасно еще не пробовал).
    </li>
    <li>
        Анонимная функция не имеет названия. Присваивается как значение переменной. Далее эту переменную
        можно вызывать как ф-ю. 
    </li>
    <li>
        Начиная с версии 8 в PHP при вызове ф-ии аргументы можно передавать по имени. 
        В команде вызова ф-ии указываем имя арг-та без символа $ и через : значение арг-та.
    </li>
</ul>
</body>
</html>