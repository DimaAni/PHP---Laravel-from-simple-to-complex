...
<p>
    Обычно операторы бывают унарными(с одним операндом) и 
    бинарыми(с двумя операндами). Также сущесвует еще один
    вариант с тремя операндами.
</p>
<p>
    Тернарный оператор в PHP позволяет выпонянть условное выраажение,
    оно выполняет одно из двух значений в зависимости от условия.
...
<p>
Если условие истинно, будет возвращено значение_если_истина, иначе – 
значение_если_ложь. Пример тернарного оператора:
</p>
<pre>
&lt;?php
    $age = 17;
...
<hr>
<br>
<h2>Вспомним что прошли по итогу во 2 главе</h2>
<ul>
    <li>
        Переменная эта именновая часть блока памяти, к которой можно обратиться
        по имени. Чтобы ее создать достаточно присвоить значение. Имя п-ой
        начинается с символа $. У самой переменной нет типа. Но есть у 
        значения на которое ссылается п-ая.
    </li>
    <li>
        К основным типам значений можно отнести такие как:
        integer - (целые числа).
        double - (действительные числа).
        string - (текст).
        boolean - (логическиек значения).
        array - (массив).
        object - (объект).
    </li>
    <li>
        Для явного преобразования используется ф-я settype().Или указывают 
        конечный тип в скобках перед преобраземым значением. Также в PHP
        есть автоматическое преобразования типов.
    </li>
    <li>
        Для конкатенации строк в PHP используется точка.
    </li>
    <li>
        Оператор присваивания возвращает значение которое совпадает со значением
        переменной(которой присваивается значение).Также сущесвуют сокращенные
        формы присваивания.
    </li>
    <li>
        Тернарный оператор возвращает значение,которое зависит от истиности 
        или ложности некоторого условия.
    </li>
    <li>
        fgets () - для считывания данных с консоли.
    </li>
</ul>
</body>
</html>