...
<h2>
    Вспоминаем что проходили в 7 главе.
</h2>

<ul>
    <li>
        Объекты создаются на основе классов и могут содержать в себе поля и методы.
    </li>
    <li>
        Описание класса начинается  со слова class после указывем имя класса и далее в фигурных
        скобках описываем класс.
    </li>
    <li>
        Поля и методы описанные внутри класса называются <strong>членами класса</strong>. Методы 
        описываются как обычные ф-ии. При описании полей значения полей можно не указывать.
    </li>
    <li>
        Для определения уровня доступа можно использовать спецификаторы уровня доступа 
        private(закрытый член класса) public(соответсвенно открытый) ну и protected (защищенный).
        Отсутствие же спецификатора уровня доступа  означает что соответствующий член класса открытый. 
    </li>
    <li>
        Для создания объекта на основе класса применяем инструкцию new. После указываем название класса. 
        Значение соответствующего выражения является ссылка на созданный объект. Эта ссылка присваивается 
        значением объектной переменной.
    </li>
    <li>
        При присваивании объектных пер-х копия объекта не создается а две переменные будут ссвлаться 
        на один и тот же объект. Если необходимо создать копию объекта в команде присваивания пишем 
        интструкцию clone.
    </li>
    <li>
        Конструктор является методом который вызывается автоматически при создании объекта. 
        Он нзывается __construct() и может иметь аргументы. Если так то значения аргументов для 
        конструктора указывается в команде создания объекта внутри пустых круглых скобках после
        имени класса.
    </li>
    <li>
        Деструктор это метод который вызывается автоматически при удалении объекта из памяти. 
        __destruct() не имеет аргументов.
    </li>
    <li>
        Класс может содержать в себе статически методы и поля. Такие члены описываются ключевым словом 
        static . Статические члены класса не привязаны к какому- конкретно объекту и они существуют
        не зависимо от того создавался ли объект соответсвующего класса.  
    </li>
    <li>
        Для обращения к полям объекта используются инструкции вида $объект->поля. Причем в названии 
        поля $ символ не указывается. Метод из объекта вызывается командой $объект->метод(аргументы). 
        Обращение к статическим членам класса выполняется в форме вида класс::$поле и класс::метод(аргументы).
    </li>
    <li>
        $this используется в теле класса как идентификатор класса при обращении к статическим полям и методам. 
    </li>
    <li>
        Существуют специальные методы которые имеют особое значение. Название таких методов указывается с 
        двойного подчеркивания. К примеру __construct. 
    </li>
</ul>
