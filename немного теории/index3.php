...
<hr>
<br>
<h3>Оператор выбора match</h3>
<p>
    Оператор match был добавлен в PHP 8.0 и представляет собой более лаконичный и 
    выразительный способ написания условных конструкций. Он позволяет сравнивать
    значение с несколькими вариантами и выполнять соответствующий блок кода. 
...
<p>
    Разница  между match и switch заключается в том, что он возвращает значение, 
    что делает его более похожим на тернарный оператор. Также match позволяет 
    использовать множественные условия и не требует использования break после каждого случая.
</p>
<p>
    Опреатор выбора match должен возвращать результат. Если происходит так, что не один
    из блоков не срабатывает(совпвадений нет и нет блока default )тогда будет ошибка. 
    Сравнение значений происходит до первого совпадения. Если оно найдено, то остальные
    значения не выполняются. 
</p>
<p>
    Если для нескольких значений выражения результат один, то значение укажем в одном блоке
    разделяя запятыми. 
</p>
<hr>
<br>
<h3>Вспомним кратко что изучили в этой главе</h3>
<ol>
<li>
    <strong>Условный оператор if</strong>  позволяет выполнять разные блоки команд. 
    Это зависит от того(истина) они или (ложь). Условия указываются в скобках после
    <strong>if</strong> , а затем указываются команды при которых условие true. 
    Команды выполняемые для ложного условия, размещаем в блоке else.
</li>
<li>
    <strong>Оператор цикла  while</strong>  позволяет многократно выполнять блок команд. 
    Они выполняются пока условие истинно. Оно указывается после ключевого 
    слова <strong>while</strong>. 
</li>
<li>
    <strong>Оператор do-while</strong>отличается от оператора while тем, что в do-while
    сначало выолняется блок команд а затем проверятся условие. 
    (В while начинается с проверки условия).
</li>
<li>
    <strong>Оператор цикла фор</strong>имеет более сложный синтаксис чем while и do-while 
    но при этом удобней и не уступает по возможностям от других операторов цикла.
</li>
<li>
    <strong>Инструкция break</strong>позволяет досрочно остановить выполнения оператора
    цикла. 
</li>
<li>
    <strong>Оаператор выбора switch</strong>позволяет выполнять разные блоки команд
    в зависимости от значения некоторого выражения. Он содержит case блоки с контрольными
    значениями. Если значения проверяемого выражения совпадают с контрольным значением, 
    выполняются соответвующие команды в case-блоке. Команды в необяательном блоке default
    срабатывают если совпадения не найдены. 
</li>
<li>
    <strong>Инструкция goto</strong>позволяет переходить к месту в коде где установлена
    специальная метка.Метка к которой выполняется переход, указывается после инструкции
    goto. Чтбы отметить место, указывется имя метки и двоеточие. 
</li>
<li>
    <strong>Оператор выбора match</strong>позволяет проверить выражение некоторого значения
    и в зависимости этого значения, возвращяет результат. То есть типо того что объеденить
    вместе оператор выбора switch и тернарного оператора. 
</li>
</ol>
</body>
</html>